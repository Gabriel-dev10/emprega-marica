...
import { ModalMapaDistrito } from "@/layouts/mapa/ModalMapaDistrito"; // Verifique se o caminho está correto
...
export function JobSearchForm() {
  ...
  return (
    <form className="w-full relative" onSubmit={(e) => e.preventDefault()}>
      <div className="flex flex-col md:flex-row items-center gap-3 md:gap-0 bg-neutral-900 border border-neutral-800 rounded-xl p-2 shadow-2xl">
        ...
        <div className="flex flex-col flex-1 w-full relative">
          <div className="flex items-center w-full px-4 py-2 hover:bg-neutral-800/50 rounded-lg transition-colors relative group">
            <MapPin
              size={20}
              className="text-text-subtle mr-3 shrink-0 group-focus-within:text-text-primary transition-colors"
            />
          ...
          <div className="md:absolute md:-bottom-6 ml-4">
            <ModalMapaDistrito onSelecionar={(val) => setDistrito(val)} />
          </div>
        </div>
